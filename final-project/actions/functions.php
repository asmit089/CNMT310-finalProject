...
function displayBookmarks() {
    ...
    if ($response['result'] === 'Success' && is_array($response['data'])) {
        ...
        foreach ($response['data'] as $bookmark) {
			$output .= '<li>';
			$output .= '<a href="' . 'visit-bookmark.php?id=' . htmlspecialchars($bookmark['bookmark_id']) . '" target="_blank">' . htmlspecialchars($bookmark['displayname']) . '</a> (Visits: ' . htmlspecialchars($bookmark['visits']) . ')';
